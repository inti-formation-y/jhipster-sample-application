...
export class EleveComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.eleveService
      ...
      .subscribe(
        (res: HttpResponse<IEleve[]>) => this.paginateEleves(res.body, res.headers),
  ...
  trackId(index: number, item: IEleve) {
    return item.id;
  }
  ...
  protected paginateEleves(data: IEleve[], headers: HttpHeaders) {
